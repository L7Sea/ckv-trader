...
import { X, ShieldCheck, Users, MessageSquare, LogIn, Trash2, CheckCircle2, User } from 'lucide-react';
...
export const AdminPanelModal: React.FC = () => {
  ...
  const formatVND = (num: number) => Math.round(num || 0).toLocaleString('vi-VN') + ' đ';

  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-4xl max-h-[85vh] p-6 shadow-2xl relative flex flex-col justify-between space-y-4">
        <button
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-white">Quản Trị Hệ Thống & Người Dùng (Admin VIP)</h3>
              ...
              <p className="text-xs text-slate-400">
                Tổng cộng <b>{allUsers.length} tài khoản</b> đã tham gia nền tảng CKV Pro Trader
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-indigo-500 hover:bg-indigo-400 text-white font-bold text-xs flex items-center gap-1.5 transition shadow-md shadow-indigo-500/20 mr-8"
          >
            <MessageSquare className="h-4 w-4" />
            <span>Mở Kênh Chat Khách Hàng</span>
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-950/80 text-slate-400 font-semibold uppercase tracking-wider sticky top-0">
              <tr>
                <th className="py-2.5 px-3 rounded-l-xl">Người Dùng</th>
                <th className="py-2.5 px-3">Email / Mã TK</th>
                <th className="py-2.5 px-3">Vai Trò</th>
                <th className="py-2.5 px-3">Tiểu Khoản</th>
                ...
                return (
                  <tr key={u.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-3 px-3 text-slate-300 font-sans">
                      TK {u.subAccount} ({u.subAccount === '06' ? 'Margin' : 'Thường'})
                    </td>
                    ...
                    <td className="py-3 px-3 text-right font-sans">
                      {!isCurrent && (
                        <button
                          onClick={() => {
                            if (window.confirm(`Chuyển sang đăng nhập tài khoản "${u.name}"?`)) {
