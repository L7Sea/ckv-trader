...
import { USER_WATCHLIST } from './MarketBoard';
import { run52PredictionAlgorithms, AlgorithmResult, SignalType } from '../services/predictionEngine';

export const FiftyAlgorithmsReport: React.FC = () => {
  const { selectedSymbol, setSelectedStock, positions } = useTradingStore();
  const [activeCategory, setActiveCategory] = useState<string>('ALL');
  const [searchTerm, setSearchTerm] = useState<string>('');

  const activeSym = selectedSymbol || (positions.length > 0 ? positions[0].symbol : 'TPB');
  const foundStock = USER_WATCHLIST.find((s) => s.symbol === activeSym);
  ...
  const prediction = useMemo(() => {
    return run52PredictionAlgorithms(activeSym, currentPrice);
  }, [activeSym, currentPrice]);

  const categories = [
    { key: 'ALL', name: 'Tất Cả (52)', count: 52 },
    { key: 'TREND', name: '📈 Xu Hướng & Động Lượng', count: 15 },
    { key: 'VOLATILITY', name: '🌊 Biến Động & Dải Đo', count: 8 },
    { key: 'VOLUME', name: '💰 Dòng Tiền & Khối Lượng', count: 8 },
    { key: 'PIVOT', name: '🎯 Price Action & Pivot', count: 8 },
    { key: 'RISK', name: '🛡️ Quản Trị Rủi Ro', count: 6 },
    { key: 'VALUATION', name: '📊 Định Giá Cơ Bản', count: 7 },
  ];
  ...
  const getSignalBadge = (sig: SignalType) => {
    switch (sig) {
      case 'STRONG_BUY':
        return <span className="px-2.5 py-1 rounded-xl bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-[11px] font-bold font-mono">MUA MẠNH</span>;
      case 'BUY':
        return <span className="px-2.5 py-1 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[11px] font-bold font-mono">MUA</span>;
      case 'NEUTRAL':
        return <span className="px-2.5 py-1 rounded-xl bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[11px] font-bold font-mono">TRUNG LẬP</span>;
      case 'SELL':
        return <span className="px-2.5 py-1 rounded-xl bg-rose-500/10 text-rose-400 border border-rose-500/20 text-[11px] font-bold font-mono">BÁN</span>;
      case 'STRONG_SELL':
        return <span className="px-2.5 py-1 rounded-xl bg-rose-500/20 text-rose-300 border border-rose-500/40 text-[11px] font-bold font-mono">BÁN MẠNH</span>;
    }
  };
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800/80 rounded-3xl p-5 shadow-sm space-y-6">
      {/* ══ HEADER: CHỌN MÃ & TỔNG HỢP 52 THUẬT TOÁN ══ */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 pb-4 border-b border-slate-800">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-black text-white uppercase tracking-wider font-sans">
                HỆ THỐNG 52 THUẬT TOÁN & CÔNG THỨC DỰ ĐOÁN ĐỊNH LƯỢNG
              </h2>
            ...
            <p className="text-xs text-slate-400">
              Tổng hợp phân tích kỹ thuật, dòng tiền, Price Action, thống kê rủi ro & định giá cơ bản
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto">
          <span className="text-xs text-slate-400 whitespace-nowrap">Chọn mã phân tích:</span>
          <select
            value={activeSym}
            onChange={(e) => {
              const s = USER_WATCHLIST.find((item) => item.symbol === e.target.value);
              setSelectedStock(e.target.value, s?.price || 14450, 'BUY');
            }}
            className="bg-slate-950 border border-slate-700 text-white font-bold font-mono text-base rounded-xl px-3 py-1.5 focus:outline-none focus:border-indigo-500 cursor-pointer w-full lg:w-48"
          >
            <option value="TPB">TPB (Vị thế nắm giữ)</option>
            {USER_WATCHLIST.map((item) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="md:col-span-4 p-5 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950/40 rounded-2xl border border-indigo-500/30 flex flex-col justify-between">
          <div>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider font-mono">
              ĐIỂM ĐỒNG THUẬN 52 THUẬT TOÁN
            </span>
            ...
            <p className="text-xs text-slate-300 mt-3 leading-relaxed">
              Mã <b>{prediction.symbol}</b> đạt mức đồng thuận <b>{prediction.overallScore}%</b> tích cực từ 52 mô hình định lượng.
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 pt-4 mt-4 border-t border-slate-800 font-mono text-center text-xs">
            <div className="p-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20">
              ...
              <b className="text-base text-emerald-400">{prediction.buyCount}</b>
            </div>
            <div className="p-2 bg-amber-500/10 rounded-xl border border-amber-500/20">
              <span className="text-[10px] text-amber-400 block">Trung Lập</span>
              <b className="text-base text-amber-400">{prediction.neutralCount}</b>
            </div>
            <div className="p-2 bg-rose-500/10 rounded-xl border border-rose-500/20">
              <span className="text-[10px] text-rose-400 block">Tín Hiệu Bán</span>
              <b className="text-base text-rose-400">{prediction.sellCount}</b>
            </div>
          </div>
        </div>

        {/* Forecast Targets Card */}
        <div className="md:col-span-8 p-5 bg-slate-950 rounded-2xl border border-slate-800 flex flex-col justify-between space-y-4">
          ...
          <div className="p-2.5 bg-slate-900 rounded-xl border border-slate-800 text-xs text-slate-300 flex items-center gap-2">
      ...
      {/* ══ BỘ LỌC 6 NHÓM CHỈ BÁO & THANH TÌM KIẾM ══ */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 font-sans"
            />
          </div>
        </div>

        {/* ══ DANH SÁCH CHI TIẾT 52 THUẬT TOÁN ══ */}
        <div className="overflow-x-auto rounded-2xl border border-slate-800/80 max-h-[500px]">
          <table className="w-full text-left text-xs font-mono">
            <thead className="sticky top-0 bg-slate-950 z-10">
              <tr className="border-b border-slate-800 text-slate-400 uppercase tracking-wider font-sans font-semibold">
                <th className="py-3 px-4">Tên Thuật Toán / Mô Hình</th>
                ...
                <th className="py-3 px-4 font-sans">Giải Thích Kỹ Thuật & Công Thức</th>
              </tr>
            </thead>
              ...
              {filteredAlgos.map((algo, index) => (
                <tr key={algo.id} className="hover:bg-slate-800/50 transition">
                  <td className="py-3 px-4 font-bold text-white">
                    <div className="flex items-center gap-2">
                      <span className="text-slate-400 text-[10px] w-5">#{index + 1}</span>
                  ...
                  <td className="py-3 px-4 font-sans text-xs text-slate-300">
                    ...
                    <span className="text-[10px] font-mono text-slate-500 block mt-0.5">
                      Công thức: <code className="text-slate-400">{algo.formula}</code>
                    </span>
                  </td>
                </tr>
              ))}
